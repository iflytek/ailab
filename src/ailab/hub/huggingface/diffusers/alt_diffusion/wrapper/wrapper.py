...
# 使用的模型
model = "BAAI/AltDiffusion-m9"
task = "alt-diffusion-pipeline"
prompt = "幻想风景, artstation"
device = "cuda"

# 定义模型的超参数和输入参数
class UserRequest(object):
    input1 = ImageBodyField(key="image", path='./sketch-mountains-input.jpg')
...
class Wrapper(WrapperBase):
    ...
    def wrapperInit(self, config: {}) -> int:
        ...
        self.pipe = AltDiffusionImg2ImgPipeline.from_pretrained(model, torch_dtype=torch.float16).to("cuda")
    ...
    def wrapperOnceExec(self, params: {}, reqData: DataListCls) -> Response:
        ...
        result = self.pipe(prompt=prompt, image=img, strength=0.75, guidance_scale=7.5).images[0]
